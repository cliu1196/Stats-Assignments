...
1. `102b_hw6_output_First_Last.Rmd` Take this R Markdown file and make the necessary edits so that it generates the requested output.

2. `102b_hw6_output_First_Last.pdf` OR  `102b_hw_6_output_First_Last.html` Your output file. This can be a PDF or an HTML file. This is the primary file that will be graded. **Make sure all requested output is visible in the output file.**
...
"I did discuss ideas related to the homework with Josephine Bruin for parts 2 and 3, with John Wooden for part 2, and with Gene Block for part 1. At no point did I show another student my code, nor did I look at another student's code."
Homework questions and text copyright Miles Chen. For personal use only. Do not distribute. Do not post or share your solutions.
...
# Method 1: PCA with the covariance matrix
...
```{r, fig.width=5, fig.height=5, error = TRUE}
# ... write your code to produce xrec
...
# PCA Method 2: PCA with the correlation matrix
...
# Using R's built in `prcomp()` functions

R has two principal components functions built-in. So far we have done PCA by combining the use of more elementary functions like `cov()`, `cor()` and `eigen()`. `prcomp()` does PCA via SVD. 
...
_Use the function `prcomp()` on the **original data** matrix `x`. Store the results in `pcresults`. Print out `pcresults` and `summary(pcresults)`. Check to see that the rotation matrix matches the results from Task 3. Use `biplot` to create a plot. (This dataset has too many observations for us to see the individual obserations.)_
...
To perform PCA of the correlation matrix using `prcomp()`, simply add the option, `scale = TRUE`

#### Task 14

_Use `prcomp()` on the original data matrix `x` using the correlation matrix. Store the results in `pcresults2`. Print out `pcresults2` and `summary(pcresults2)`. Verify that the rotation matrix is equivalent to the Qr matrix from Task 8._
